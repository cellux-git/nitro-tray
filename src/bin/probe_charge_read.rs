...
use nitro_tray::charge::{ChargeRow, SmartChargeAdapter, CLASS_NAME};
...
fn print_row(label: &str, row: Result<Option<ChargeRow>, AdapterError>) {
    match row {
        Ok(Some(row)) => println!(
            ...
            row.function_list,
            row.status.iter().map(u8::to_string).collect::<Vec<_>>().join(","),
            row.return_value.map(|rv| rv.to_string()).unwrap_or_default()
        ),
...
fn main() {
    ...
    for battery in 0..4u8 {
        for query in 0..4u8 {
            print_row(&format!("  b{battery} q{query}"), adapter.read_row(battery, query));
        }
    }
