//! Power-plan seam, split by platform (linux-port ticket 02).
//!
...
//!
//! Linux: `PowerApi` is a stub whose `PlanApi` implementation reports
//! "unavailable" — the sysfs governor/EPP/boost backend (plan table 1:1,
//! no external processes) lands in ticket 05.
...
pub enum PowerError {
    ...
    NotActive,
    /// The plan API is not available on this platform (Linux stub, ticket
    /// 02; the sysfs backend lands in ticket 05).
    Unavailable,
}
...
/// Windows is the in-process Win32 power API; Linux is a stub reporting
/// `PowerError::Unavailable` until the ticket-05 sysfs backend lands.
pub struct PowerApi;
...
/// Linux stub (linux-port ticket 02): every plan operation reports
/// `PowerError::Unavailable` — the entry point logs the failures and the app
/// core keeps running (never-terminal degrade). The sysfs
/// governor/EPP/boost backend lands in ticket 05.
#[cfg(target_os = "linux")]
impl PlanApi for PowerApi {
    fn ensure_nitro_plans(&self) -> Result<(), PowerError> {
    ...
    fn set_active_plan(&self, _plan: &str) -> Result<(), PowerError> {
        Err(PowerError::Unavailable)
    }
